            ...
            <MainLayout className='flex-1 flex flex-col gap-[10em] sm:pt-[8em] pt-[3em] px-4'>
                <div className='w-full flex justify-center items-center p-1 gap-20 flex-wrap gap-y-10'>
                    <div className='flex flex-col gap-4'>
                        <h1 className="max-w-[700px] text-3xl font-medium text-[#3c3d51]">{t('goShop')}</h1>
                        <Link className="w-fit text-white px-5 py-2 bg-blue-600 hover:bg-blue-500 text-base rounded-md" to={'/shop'}>{t('buttonGoShop')}</Link>
                    </div>
                ...
                <div className='w-full flex justify-center items-center p-1 gap-4 flex-wrap'>
                    ...
                    <h1 className='max-w-[700px] sm:text-3xl text-2xl text-left font-medium text-[#3c3d51]'>{t('welcome')}</h1>
                </div>
                { /** News block */ }
                <div className='w-full flex flex-col gap-6'>
                    <div className='relative w-full'>
                        <hr className='w-full' />
                        <h1 className='absolute left-1/2 transform -translate-x-1/2 text-gray-800 mt-[-0.8em] sm:text-2xl text-lg bg-[var(--background--)] sm:w-fit w-[205px] px-4'>{t('allNews')}</h1>
                    </div>
                ...
                <div className='w-full flex justify-center items-center p-1 gap-8 flex-wrap'>
                    <div className='w-full max-w-[800px] flex flex-col gap-4'>
                        <h1 className='text-4xl font-semibold text-gray-700'>{t('homeAboutProject')}</h1>
                ...
                <div className='w-full flex justify-center items-center p-1 gap-8 flex-wrap-reverse'>
                    ...
                    <div className='w-full max-w-[800px] flex flex-col gap-3'>
                        <h1 className='text-4xl font-semibold text-gray-700'>{t('homeProjectGoal')}</h1>
                        <p className='text-xl'>{t('homeProjectGoalInfo')}</p>
                        <button onClick={() => document.getElementById('result')?.scrollIntoView({
                            behavior: 'smooth'
                        })} className='flex items-center sm:gap-2 gap-4 p-2 w-fit rounded-md'>
                            <Icon16ArrowRightOutline width={20} height={20} color='orange' />
                            <p className='font-medium text-gray-800 text-left'>{t('homeProjectGoalButton')}</p>
                        </button>
                    </div>
                </div>
                { /** Finance 
                <div className='flex flex-col'>
                    <div className='flex justify-center items-center gap-2'>
                        ...
                        <h1 className='text-2xl font-semibold text-gray-700 text-center'>{t('homeFinance')}</h1>
                    </div>
                    <h3 className='text-center text-xl mt-3'>{t('homeFinanceInfo')}</h3>
                    <div className='flex flex-wrap justify-center gap-10 mt-[3em]'>
                        <div className='border shadow-xl p-3 rounded-2xl w-[300px] h-[150px] flex flex-col justify-center items-center gap-2'>
                            ...
                            <h2 className='text-xl text-gray-600'>{t('homeFinanceYear2023')}</h2>
                        </div>
                        <div className='border shadow-xl p-3 rounded-2xl w-[300px] h-[150px] flex flex-col justify-center items-center gap-2'>
                            <h1 className='text-5xl text-gray-800 font-semibold'>60 000₸</h1>
                            <h2 className='text-xl text-gray-600'>{t('homeFinanceYear2024')}</h2>
                        </div>
                        <div className='border shadow-xl p-3 rounded-2xl w-[300px] h-[150px] flex flex-col justify-center items-center gap-2'>
                            <h1 className='text-5xl text-gray-800 font-bold'>15 000₸</h1>
                            <h2 className='text-xl text-gray-600'>{t('homeFinanceYear2025')}</h2>
                        </div>
                    </div>
                </div> */ }
                { /** Result */ }
                <div id="result" className='w-full flex justify-center items-center p-1 sm:gap-8 gap-2 flex-wrap'>
                    <div className='w-full max-w-[800px] flex flex-col gap-3'>
                        <h1 className='text-4xl font-semibold text-gray-700'>{t('homeRelase')}</h1>
                        <p className='text-xl'>{t('homeRelaseInfo')}</p>
                        <button onClick={() => document.getElementById('blocks')?.scrollIntoView({
                            behavior: 'smooth'
                        })} className='flex items-center gap-2 p-2 w-fit rounded-md'>
                            <Icon28ChevronRightCircle width={24} height={24} color='blue' />
                            <p className='font-medium text-gray-800'>{t('homeRelaseButton')}</p>
                        </button>
                    </div>
